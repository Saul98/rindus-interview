...
import com.rindus.interview.application.usecase.CreateUserUseCase;
import com.rindus.interview.application.usecase.GetUserUseCase;
...
public class UsersResource {
  ...
  public Response create(@Valid CreateUserPayload payload, @Context UriInfo uriInfo) {
    LOG.debugf(
        "HTTP POST /users - Creating user name=%s, email=%s", payload.name(), payload.email());
    ...
    LOG.infof("User created via API with id=%s", result.id());
    return Response.created(location).entity(UserResponseDto.from(result)).build();
  }
  ...
  public Response get(@PathParam("id") UUID id) {
    LOG.debugf("HTTP GET /users/%s - Retrieving user", id);
